        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/">
            <div className="flex items-center gap-2 cursor-pointer group">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-primary group-hover:scale-105 transition-transform">
              ...
              return (
                <Link key={link.name} href={link.path}>
                  <div className="relative cursor-pointer py-1 group">
                    <span className={`text-sm font-medium transition-colors ${isActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground"}`}>
                      {link.name}
                    </span>
                    {isActive && (
                      <motion.div 
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <a href="https://wa.me/905555555555" target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-green-500 transition-colors p-2">
              <FaWhatsapp className="w-5 h-5" />
            </a>
            <Link href="/contact">
              <div className="bg-primary text-primary-foreground px-6 py-2.5 text-sm font-medium cursor-pointer hover:bg-primary/90 transition-colors uppercase tracking-wider rounded-none">
                Teklif Al
              </div>
            </Link>
          </div>
              ...
              {navLinks.map((link) => (
                <Link key={link.name} href={link.path}>
                  <div 
                    ...
                    className={`text-2xl font-serif cursor-pointer ${location === link.path ? "text-primary" : "text-foreground"}`}
                  >
                    {link.name}
                  </div>
                </Link>
